...
async function getEvents(emit) {
  ...
  if (fs.existsSync(histPath)) {
    await streamJsonl(histPath, (h) => {
      try {
        const ts =
          typeof h.timestamp === "number"
            ? new Date(h.timestamp).toISOString()
            : h.timestamp;
        if (!ts || !h.display) return;
        emit({
          agent: NAME,
          timestamp: ts,
  ...
  await Promise.all(
    projDirs.map(async (dir) => {
      const projDirPath = path.join(projectsDir, dir.name);
      const files = await findFiles(projDirPath, ".jsonl");
      await Promise.all(
...
async function readStatsCache() {
  return await readJsonSafe(path.join(dataDir(), "stats-cache.json"));
}
